...
void matrix_print_ascii(Matrix *m) {
    const char* gradient = " .:-=+*#%@";
    int gradient_length = 10;
    printf("Rows: %d, Cols: %d\n", m->rows, m->cols);
    for (int i=0; i < m->rows; i++) {
        for (int j=0; j < m->cols; j++) {
            int index = (int)(m->entries[i][j] * gradient_length);
            if (index >= gradient_length) index = gradient_length - 1;
...
// Axis = 0 -> Column vector, 1 -> Row Vector
